...
class DecoderEventTransformer(nn.Module):
    def __init__(self,input_size,output_size,hidden_size=128,num_heads=8,number_layers=4,dropout=0.1):
        super().__init__()

        self.em                = nn.Sequential(nn.Linear(input_size, hidden_size), nn.ReLU())
        self.transformer_layer = nn.TransformerEncoderLayer(d_model = hidden_size, nhead=num_heads,batch_first=True)
        self.transformer       = nn.TransformerEncoder(self.transformer_layer, number_layers)
    ...
    def forward(self, X, attention):
        #pdb.set_trace()
        X_em         = self.em(X)
        attn_applied = torch.bmm(attention.unsqueeze(1), X_em).squeeze(1)
        attn_applied = self.dropout(attn_applied)    # dropout
        output       = self.transformer(attn_applied).relu() # Do we need relu() here?
        output       = self.dropout(output)          # dropout
        output       = self.out(output)
        output       = F.log_softmax(output,dim=1) # Jia change, if CrossEntropyloss
