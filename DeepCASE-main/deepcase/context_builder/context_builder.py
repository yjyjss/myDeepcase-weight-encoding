...
from .decoders  import DecoderAttention, DecoderEvent, DecoderEventTransformer
...
class ContextBuilder(nn.Module):
    ...
    def __init__(self, input_size, output_size, hidden_size=128, num_layers=1,
        ...
        # Create transformer event decoder
        self.decoder_event = DecoderEventTransformer(
            input_size  = input_size,
            output_size = output_size,
            dropout     = 0.1,
        )
    ...
    def fit(self, X, y,labels, epochs=10, batch_size=128, learning_rate=0.01,
        ...
        # Resolve the unbanlance problem in the data
        class_counts = torch.bincount(y.squeeze(1))
        class_weights = 1. /class_counts.float()
        sample_weights = class_weights[y.squeeze(1)]
        ...
        for epoch in range(1, epochs+1):
            try:
                ...
                if verbose:
                    data = tqdm(data,
                        desc="[Epoch {:{width}}/{:{width}} loss={:.6f}]"
